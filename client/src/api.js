...
export const api = {
  ...
  async createTeacher(token, name, email, password, code, courseName) {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/manage-users`, {
      ...
      body: JSON.stringify({ type: 'teacher', name, email, password, course_code: code, course_name: courseName })
    });
  ...
  async createStudent(token, name, email, roll, password, code) {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/manage-users`, {
      method: 'POST',
      headers: { ...getHeaders(token), 'x-mark-fn-api-key': MARK_FN_KEY },
      body: JSON.stringify({ type: 'student', name, email, roll_number: roll, password, course_code: code })
    });
    return { ...await res.json(), ok: res.ok };
  },
